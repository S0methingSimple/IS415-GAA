...
colnames(piv_full_df_mys)
```

```{r}
corrplot.mixed(cor(st_drop_geometry(piv_full_df_mys)[, 2:14]),
         lower = "ellipse", 
               upper = "number",
               tl.pos = "lt",
               diag = "l",
...
```{r}
write_rds(piv_full_df_mys, "./data/rds/pivot_df.rds")
```

```{r}
piv_full_df_mys <- read_rds("./data/rds/pivot_df.rds")
```
...
```{r}
clust_rownames <- piv_full_df_mys$state_district
og_clust_vars <- piv_full_df_mys %>% 
...
grid.arrange(grobs = hist_list, ncol = 5, nrow = 3)
```
...
## 3.2 Selecting Clusters

### 3.2.1 Gap Statistics

```{r}
set.seed(12345)
gap_stat <- clusGap(clust_vars, 
                    FUN = kmeans, 
                    nstart = 25, 
                    K.max = 15, 
                    B = 50)
...
### 3.2.2 Visualisign Gap Statistics
...
```{r}
nbc <- NbClust(clust_vars, distance = "euclidean", min.nc = 2, max.nc = 15, method = "kmeans")

print(nbc, method = "firstmax")
```

```{r}
fviz_nbclust(clust_vars, FUNcluster = pam)
```

## 3.2 Hierarchical Clustering

### 3.2.2 Compute Prox Mat

```{r}
proxmat <- dist(clust_vars, method = 'euclidean')
```

### 3.2.3 Compute Cluster

#### 3.2.3.1 Selecting Clustering Method
...
#### 3.2.3.2 Perform Clustering

```{r}
hclust_ward <- hclust(proxmat, method = 'ward.D')
```

#### 3.2.3.3 Visualizing Cluster
...
### 3.2.5 Compute Cluster (Geo Constrained)

#### 3.2.5.1 Compute distmat
...
#### 3.2.5.2 Compute Choicalpha

```{r}
cr <- choicealpha(proxmat, distmat, range.alpha = seq(0, 1, 0.1), K=13, graph = TRUE)
```

#### 3.2.5.3 Perform Clustering

```{r}
hclust_geo <- hclustgeo(proxmat, distmat, alpha = 0.1)
```

#### 3.2.5.4 Visualizing Cluster
...
```{r}
groups <- as.factor(cutree(hclust_geo, k=13))
mys_sf_geo_cluster <- cbind(piv_full_df_mys, as.matrix(groups)) %>%
  rename(`CLUSTER`=`as.matrix.groups.`)
qtm(mys_sf_geo_cluster, "CLUSTER")
```

##### Comparison

```{r}
clust <- qtm(mys_sf_cluster, "CLUSTER")
clust2 <- qtm(mys_sf_geo_cluster, "CLUSTER")

tmap_arrange(clust, clust2, ncol = 2)

```
:::

## 3.3 Skater Clustering

### 3.3.1 Generating Neighbours
...
```{r}
coords <- st_coordinates(st_centroid(st_geometry(piv_full_df_mys)))
...
### 3.3.2 Fixing Neighbours

```{r}
components <- n.comp.nb(nb)
which(components$comp.id == 2)
piv_full_df_mys$state_district[which(components$comp.id == 2)]
which(components$comp.id == 3)
piv_full_df_mys$state_district[which(components$comp.id == 3)]
```
...
```{r}
indices_langkawi_boat <- which(piv_full_df_mys$state_district %in% c("PERLIS-PERLIS", "KEDAH-KOTA SETAR", "PULAU PINANG-TIMUR LAUT"))

indices_langkawi_boat
piv_full_df_mys$state_district[indices_langkawi_boat]
```
...
piv_full_df_mys$state_district[indices_pulau_pinang]
```
...
qtm(crime_mys_spatialcluster, "SP_CLUSTER")
```
:::

## 3.4 Intepreting Results


### 3.4.1 Comparing Clusters

```{r}
clust <- qtm(crime_mys_spatialcluster, "SP_CLUSTER")
clust2 <- qtm(mys_sf_geo_cluster, "CLUSTER")

tmap_arrange(clust, clust2, ncol = 2)
```

### 3.4.2 Parral Cordinates

```{r}
ggparcoord(data = crime_mys_spatialcluster, 
           columns = c(2:8), 
           scale = "globalminmax",
           alphaLines = 0.2,
           boxplot = TRUE, 
...
### 3.4.3 Cluster Profiling

```{r}
crime_mys_spatialcluster %>% 
  st_drop_geometry() %>%
  group_by(SP_CLUSTER) %>%
