...
### 3.2.2 Visualizing Gap Statistics

```{r}
fviz_gap_stat(gap_stat)
```

### 3.2.3 Compute NB Clust
...
hclust_ward_cluster <- cbind(piv_full_df_mys, as.matrix(groups)) %>%
  rename(`CLUSTER`=`as.matrix.groups.`)
hclust_ward_plot <- qtm(hclust_ward_cluster, "CLUSTER")
hclust_ward_plot
```
:::
...
hclust_geo_cluster <- cbind(piv_full_df_mys, as.matrix(groups)) %>%
  rename(`CLUSTER`=`as.matrix.groups.`)
hclust_geo_plot <- qtm(hclust_geo_cluster, "CLUSTER")
hclust_geo_plot
```
...
skater_cluster <- cbind(piv_full_df_mys, as.factor(groups_mat)) %>%
  rename(`CLUSTER`=`as.factor.groups_mat.`)
skater_plot <- qtm(skater_cluster, "CLUSTER")
skater_plot
```
:::
...
```{r}
tmap_arrange(skater_plot, hclust_geo_plot, ncol = 2)
```

### 3.4.2 Parral Coordinates
