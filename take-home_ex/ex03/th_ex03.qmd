...
However we do not have labor data, hence we will use the national average
 "PERLIS-PERLIS"
...
```{r}
corrplot.mixed(cor(st_drop_geometry(piv_full_df_mys)[, 4:19]),
         lower = "ellipse", 
               upper = "number",
               tl.pos = "lt",
               diag = "l",
...
Since the variable `lf` is highly correlated with `lf_employed`, `lf_unemployed` and `lf_outside` we can drop them from the dataframe

```{r}
piv_full_df_mys <- piv_full_df_mys %>%
  select(-lf_employed, -lf_unemployed, -lf_outside)
colnames(piv_full_df_mys)
```


```{r}
corrplot.mixed(cor(st_drop_geometry(piv_full_df_mys)[, 4:16]),
         lower = "ellipse", 
               upper = "number",
               tl.pos = "lt",
               diag = "l",
