...
1. Load mys, crime and pop
1.1 Extract year and convert state district to upper

2. Fix state level
2.1 Crime data
2.1.1 Take out west mys, district (all) and type (all)
2.2.1 Rename KL 
2.2 Population data
2.2.1 Take out west mys
2.2.1 Rename KL 
2.3 Mys Sf data
2.3.1 Filter out west mys
2.2.1 Rename KL, Purtajaya (into selangor)

3. Fix district level
3.1 Crime data
3.1.1 Compare diff
3.1.2 Remap the district
3.2 Population data
3.2.1 Compare diff
3.2.2 Remap the district

4. Join crime, pop and mys
4.1 Compute crime per capita
4.2 Scoping down for type and final clean
...
### 2.3.4 Checking for Mismatch (State-District) 
...
### 2.3.5 Cleaning (State-District)

JOHOR
1.    ISKANDAR PUTERI -> JOHOR BAHRU
2.    NUSAJAYA -> JOHOR BAHRU
3.    JOHOR BAHRU SELATAN -> JOHOR BAHRU
4.    JOHOR BAHRU UTARA -> JOHOR BAHRU
5.    SERI ALAM -> JOHOR BAHRU
...
sort(unique((mys_sf %>% filter(ADM1_EN == "NEGERI SEMBILAN"))$ADM2_EN))
sort(unique((crime_df %>% filter(state == "NEGERI SEMBILAN"))$district))

print("MELAKA:")
unique((mys_sf %>% filter(ADM1_EN == "MELAKA"))$ADM2_EN)
unique((crime_df %>% filter(state == "MELAKA"))$district)

print("JOHOR:")
sort(unique((mys_sf %>% filter(ADM1_EN == "JOHOR"))$ADM2_EN))
sort(unique((crime_df %>% filter(state == "JOHOR"))$district))
```

### 2.3.6 Performing join (South)

```{r}
south_states = c("JOHOR", "NEGERI SEMBILAN", "MELAKA")
south_mys_sf <- mys_sf %>% filter(ADM1_EN %in% south_states)

crime_df_mys <- crime_df %>% 
  filter(state %in% south_states &
         year >= 2019 & year <= 2022) %>%
  left_join(south_mys_sf, by = c("district" = "ADM2_EN")) %>%
