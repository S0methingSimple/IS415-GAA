...
2.    Labour data is missing PERLIS, KUALA LUMPUR and PUTRAJAYA
...
However we do not have labor data for the following state-districts, hence we will use the national average:
 "PERLIS-PERLIS"
...
```{r}
clust_vars_df <- as.data.frame(clust_vars)
s <- ggplot(data=clust_vars_df, 
...
```{r}
hclust_geo <- hclustgeo(proxmat, distmat, alpha = 0.4)
```
