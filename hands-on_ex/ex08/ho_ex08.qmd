...
Terminology of output features:
-   Condition Number: Evaluates local collinearity; high values (over 30) indicate unreliable results.
-   Local R2: Measures local model fit; low values suggest poor performance.
-   Predicted: Estimated y values computed by GWR.
-   Residuals: Differences between observed and fitted y values; standardized residuals have mean 0 and standard deviation 1.
-   Coefficient Standard Error: Measures reliability of coefficient estimates; small values indicate high confidence, large values may indicate local collinearity issues.
